...
#include <omp.h>
#include "stable_interp.h" //para el get_ms_time()

#define ESTM_2D_EPSABS 0.008
#define ESTM_2D_MAX_ITER 300
#define ESTM_4D_EPSABS 0.002
#define ESTM_4D_MAX_ITER 200
...
double stable_loglikelihood(StableDist *dist, double *data, const unsigned int length)
{
    ...
    stable_pdf_interp(dist, data, length, pdf);
...
double stable_loglike_p(stable_like_params *params)
{
    ...
    stable_pdf_interp(params->dist, params->data, params->length, params->pdf);
...
int stable_fit_iter_whole(StableDist *dist, const double * data, const unsigned int length)
{
	...
	//set_expanded(theta, dist->alpha, dist->beta, dist->sigma, dist->mu_1);
    	set_expanded(theta, dist->alpha, dist->beta, dist->sigma, dist->mu_0);
